...
setTextContentsFP setTextContents = NULL;
selectTextRangeFP selectTextRange = NULL;
SelectAllAccessibleSelectionFromContextFP SelectAllAccessibleSelectionFromContext = NULL;
GetAccessibleSelectionCountFromContextFP GetAccessibleSelectionCountFromContext = NULL;
GetAccessibleSelectionFromContextFP GetAccessibleSelectionFromContext = NULL;
AddAccessibleSelectionFromContextFP AddAccessibleSelectionFromContext = NULL;
...
Local<Object> VMAccessibleContextToObject(Isolate* isolate, long vmID, AccessibleContext ac)
{
...
void Exports_GetAccessibleContextFromHWND(const FunctionCallbackInfo<Value>& args) {
	...
	args.GetReturnValue().Set(VMAccessibleContextToObject(isolate, vmID, ac));
}
...
Local<Object> AccessibleContextInfoToObject(Isolate* isolate, long vmID, AccessibleContext ac, AccessibleContextInfo *info)
{
	Local<Object> obj = v8::Object::New(isolate);

	obj->Set(v8::String::NewFromUtf8(isolate, "vmac"), VMAccessibleContextToObject(isolate, vmID, ac));
...
void Exports_GetAccessibleContextInfo(const FunctionCallbackInfo<Value>& args) {
	...
	args.GetReturnValue().Set(AccessibleContextInfoToObject(isolate, (long)vmID->Value(), getAC(isolate, obj), &aci));
}
...
void Exports_GetAcessContextByArray(const FunctionCallbackInfo<Value>& args) {
	...
	args.GetReturnValue().Set(AccessibleContextInfoToObject(isolate, (long)vmID->Value(), childAc, &aci));
}
...
void Exports_FindAccessContext(const FunctionCallbackInfo<Value>& args) {
	...
	args.GetReturnValue().Set(AccessibleContextInfoToObject(isolate, (long)vmID->Value(), ac, &aci));
}

void Exports_setTextContents(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = Isolate::GetCurrent();
	HandleScope scope(isolate);

	if((!bInited) || setTextContents == NULL || (!args[0]->IsObject()) || (!args[1]->IsString()))
	{
		args.GetReturnValue().Set(Boolean::New(isolate, false));
		return;
	}
	...
	args.GetReturnValue().Set(Boolean::New(isolate, 
		setTextContents((long)vmID->Value(), getAC(isolate, obj), Utf8ToUtf16(*utfValue).c_str())));
}

void Exports_selectTextRange(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = Isolate::GetCurrent();
	HandleScope scope(isolate);

	if((!bInited) || selectTextRange == NULL || (!args[0]->IsObject()) || 
	...
	Local<Object> obj = Local<Object>::Cast(args[0]);
	Local<Uint32> vmID = Local<Uint32>::Cast(obj->Get(String::NewFromUtf8(isolate, "vmID")));
	Local<Int32> startIndex = Local<Int32>::Cast(args[1]);
	Local<Int32> endIndex = Local<Int32>::Cast(args[2]);
	args.GetReturnValue().Set(Boolean::New(isolate, 
		selectTextRange((long)vmID->Value(), getAC(isolate, obj), startIndex->Value(), endIndex->Value())));
}

void Exports_SelectAllAccessibleSelectionFromContext(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = Isolate::GetCurrent();
	HandleScope scope(isolate);

	if((!bInited) || SelectAllAccessibleSelectionFromContext == NULL || (!args[0]->IsObject()))
	{
		args.GetReturnValue().Set(Boolean::New(isolate, false));
		return;
	}
	...
	Local<Object> obj = Local<Object>::Cast(args[0]);
	Local<Uint32> vmID = Local<Uint32>::Cast(obj->Get(String::NewFromUtf8(isolate, "vmID")));
	SelectAllAccessibleSelectionFromContext((long)vmID->Value(), getAC(isolate, obj));
	args.GetReturnValue().Set(Boolean::New(isolate, true));
}

void Exports_GetAccessibleSelectionCountFromContext(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = Isolate::GetCurrent();
	HandleScope scope(isolate);

	if((!bInited) || GetAccessibleSelectionCountFromContext == NULL || (!args[0]->IsObject()))
	{
		args.GetReturnValue().Set(Int32::New(isolate, -1));
		return;
	}

	Local<Object> obj = Local<Object>::Cast(args[0]);
	Local<Uint32> vmID = Local<Uint32>::Cast(obj->Get(String::NewFromUtf8(isolate, "vmID")));
	args.GetReturnValue().Set(Int32::New(isolate, GetAccessibleSelectionCountFromContext((long)vmID->Value(), getAC(isolate, obj))));
}

void Exports_GetAccessibleSelectionFromContext(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = Isolate::GetCurrent();
	HandleScope scope(isolate);

	if((!bInited) || GetAccessibleSelectionFromContext == NULL || (!args[0]->IsObject()) || (!args[1]->IsNumber()))
	{
		args.GetReturnValue().Set(Int32::New(isolate, -1));
		return;
	}
	...
	Local<Object> obj = Local<Object>::Cast(args[0]);
	Local<Uint32> vmID = Local<Uint32>::Cast(obj->Get(String::NewFromUtf8(isolate, "vmID")));
	Local<Int32> Index = Local<Int32>::Cast(args[1]);

	AccessibleContextInfo aci;
	AccessibleContext ac = GetAccessibleSelectionFromContext((long)vmID->Value(), getAC(isolate, obj), Index->Value());
	if(ac == 0 || (!GetAccessibleContextInfo((long)vmID->Value(), ac, &aci)))
	{
...
void Exports_AddAccessibleSelectionFromContext(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = Isolate::GetCurrent();
	HandleScope scope(isolate);

	if((!bInited) || AddAccessibleSelectionFromContext == NULL || (!args[0]->IsObject()) || (!args[1]->IsNumber()))
	{
		args.GetReturnValue().Set(Boolean::New(isolate, false));
		return;
	}
	...
	Local<Uint32> vmID = Local<Uint32>::Cast(obj->Get(String::NewFromUtf8(isolate, "vmID")));
	Local<Int32> Index = Local<Int32>::Cast(args[1]);
	AddAccessibleSelectionFromContext((long)vmID->Value(), getAC(isolate, obj), Index->Value());
	args.GetReturnValue().Set(Boolean::New(isolate, true));
}
...
void Init(Handle<Object> exports) {
  ...
  NODE_SET_METHOD(exports, "setTextContents", Exports_setTextContents);
  NODE_SET_METHOD(exports, "selectTextRange", Exports_selectTextRange);
  NODE_SET_METHOD(exports, "SelectAllAccessibleSelectionFromContext", Exports_SelectAllAccessibleSelectionFromContext);
  NODE_SET_METHOD(exports, "GetAccessibleSelectionCountFromContext", Exports_GetAccessibleSelectionCountFromContext);
  NODE_SET_METHOD(exports, "GetAccessibleSelectionFromContext", Exports_GetAccessibleSelectionFromContext);
  NODE_SET_METHOD(exports, "AddAccessibleSelectionFromContext", Exports_AddAccessibleSelectionFromContext);
